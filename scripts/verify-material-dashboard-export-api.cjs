...
const bomPreviewPartCode = 'VERIFY-MDB-PART-STABLE';
const bomPreviewPartName = '零件控制面板 BOM 摘要验证零件';
const bomPreviewProjectModel = 'VERIFY-MDB-STABLE';
const bomPreviewBomPrefix = 'VERIFY-MDB-BOM-STABLE';
...
async function cleanupDashboardBomPreviewData() {
  await prisma.modelBomLine.updateMany({
    where: {
      OR: [
        { material: { partCode: { equals: bomPreviewPartCode, mode: 'insensitive' } } },
        { bom: { bomName: { startsWith: bomPreviewBomPrefix } } }
      ]
    },
    data: { status: 'DISABLED' }
  });
  await prisma.modelBom.updateMany({
    where: { bomName: { startsWith: bomPreviewBomPrefix } },
    data: { status: 'DISABLED', isCommon: false, commonSortOrder: null }
  });
  await prisma.material.updateMany({
    where: { partCode: { equals: bomPreviewPartCode, mode: 'insensitive' } },
    data: { status: 'DISABLED' }
  });
}

async function createDashboardBomPreviewData() {
  const material = await prisma.material.upsert({
    where: { partCode: bomPreviewPartCode },
    update: {
      partName: bomPreviewPartName,
      unit: '件',
      partSpecification: 'dashboard-bom-preview',
      status: 'ENABLED'
    },
    create: {
      partCode: bomPreviewPartCode,
      partName: bomPreviewPartName,
      unit: '件',
      partSpecification: 'dashboard-bom-preview',
      status: 'ENABLED'
  ...
  for (let index = 1; index <= 25; index += 1) {
    ...
    const bomName = `${bomPreviewBomPrefix}-${suffix}`;
    const bom = await prisma.modelBom.upsert({
      where: {
        bomName_customerScopeKey_projectModelScopeKey: {
          bomName,
          customerScopeKey: 'ALL',
          projectModelScopeKey: bomPreviewProjectModel.toUpperCase()
        }
      },
      update: {
        projectModel: bomPreviewProjectModel,
        customerScopeMode: 'ALL',
        status: 'ENABLED',
        isCommon: index <= 3,
        commonSortOrder: index <= 3 ? index : null,
        remark: '临时回归数据：验证零件控制面板 BOM 名称只返回摘要和总数。'
      },
      create: {
        bomName,
        projectModel: bomPreviewProjectModel,
    ...
    const existingLine = await prisma.modelBomLine.findFirst({
      where: { bomId: bom.id, materialId: material.id, sortOrder: index },
      orderBy: { createdAt: 'asc' }
    });
    if (existingLine) {
      await prisma.modelBomLine.update({
        where: { id: existingLine.id },
        data: lineData
      });
    } else {
      await prisma.modelBomLine.create({
        data: {
          bomId: bom.id,
...
async function assertDashboardBomNamePreviewContract() {
  const result = await requestJson(
    `/materials/dashboard?keyword=${encodeURIComponent(bomPreviewPartCode)}&projectModel=${encodeURIComponent(
      bomPreviewProjectModel
    )}&includeTestFixtures=true&sortBy=PART_CODE&sortOrder=ASC&limit=5&offset=0`
  );
  const matched = (result.items || []).find(
    (row) => String(row.partCode || '').toLocaleLowerCase() === bomPreviewPartCode.toLocaleLowerCase()
  );
  assert(matched, `material dashboard must include temporary BOM preview part ${bomPreviewPartCode}`);
  const bomNames = matched.bomNames || [];
  const bomStructureDetails = matched.bomStructureDetails || [];
  assert(Array.isArray(bomNames), 'material dashboard BOM preview row must expose bomNames array');
  assert(
    bomNames.length === 20,
    `material dashboard bomNames preview must keep first 20 items when 25 BOMs exist, actual ${bomNames.length}`
  );
  assert(
    matched.bomNameCount === 25,
    `material dashboard bomNameCount must keep the real total 25, actual ${matched.bomNameCount}`
  );
  assert(Array.isArray(bomStructureDetails), 'material dashboard BOM preview row must expose bomStructureDetails array');
  assert(
    bomStructureDetails.length === 10,
    `material dashboard bomStructureDetails preview must keep first 10 rows when 25 BOM rows exist, actual ${bomStructureDetails.length}`
  );
  assert(
    matched.bomStructureDetailCount === 25,
    `material dashboard bomStructureDetailCount must keep the real total 25, actual ${matched.bomStructureDetailCount}`
  );
  return {
    bomNamePreviewLength: bomNames.length,
    bomNameCount: matched.bomNameCount,
    bomStructurePreviewLength: bomStructureDetails.length,
    bomStructureDetailCount: matched.bomStructureDetailCount
  };
}
...
async function assertDashboardBomStructureExportContract() {
  const workbook = await fetchDashboardWorkbook(
    `/materials/dashboard/export?keyword=${encodeURIComponent(bomPreviewPartCode)}&projectModel=${encodeURIComponent(
      bomPreviewProjectModel
    )}&includeTestFixtures=true&sortBy=PART_CODE&sortOrder=ASC`
  );
  const worksheet = workbook.worksheets[0];
  assert(worksheet, 'material dashboard BOM structure export regression must receive a worksheet');
  ...
  assert(partCodeColumn && bomStructureColumn && bomReviewColumn, '零件管理导出缺少 BOM 结构核对列');
  let matchedRow = null;
  worksheet.eachRow((row) => {
    if (row.number <= headerRow.number) {
      return;
    }
    if (String(row.getCell(partCodeColumn).text || '').trim() === bomPreviewPartCode) {
      matchedRow = row;
    }
  });
  assert(matchedRow, `material dashboard export must include temporary BOM preview part ${bomPreviewPartCode}`);
  const structureText = String(matchedRow.getCell(bomStructureColumn).text || '').trim();
  const reviewText = String(matchedRow.getCell(bomReviewColumn).text || '').trim();
  assert(structureText.includes(bomPreviewBomPrefix), 'material dashboard export BOM structure must include BOM names, not only short structure labels');
  assert(structureText.includes('单独零件'), 'material dashboard export BOM structure must include structure labels');
  assert(structureText.includes('25 个结构'), `material dashboard export BOM structure must preserve total count, actual ${structureText}`);
...
async function assertDashboardCustomerScopeExportContract(allDashboard) {
  ...
  assert(allScopeRow, 'material dashboard customer scope export regression needs an ALL customer scope row');
  assert(historyScopeRow, 'material dashboard customer scope export regression needs an order-history customer scope row');

  const workbook = await fetchDashboardWorkbook('/materials/dashboard/export?sortBy=PART_CODE&sortOrder=ASC&status=ALL');
  const worksheet = workbook.worksheets[0];
  assert(worksheet, 'material dashboard customer scope export regression must receive a worksheet');
  const { headerRow, result: headers } = headerIndexMap(worksheet);
  const partCodeColumn = headers.get('零件编码');
  const customerScopeColumn = headers.get('适用客户');
  assert(partCodeColumn && customerScopeColumn, '零件管理导出缺少零件编码或适用客户列');

  const exportedRowsByPartCode = new Map();
  worksheet.eachRow((row) => {
    if (row.number <= headerRow.number) {
      return;
    }
    ...
    if (partCode) {
      exportedRowsByPartCode.set(partCode, row);
    }
  });

  const allScopeExportRow = exportedRowsByPartCode.get(allScopeRow.partCode);
  ...
  const historyScopeExportRow = exportedRowsByPartCode.get(historyScopeRow.partCode);
  ...
  assert(
    historyScopeText.startsWith('仅订单历史'),
    `ORDER_HISTORY customer scope export must stay marked as order history, actual ${historyScopeText}`
  );
  return {
...
async function assertDashboardExport(path, expectedScopeTexts) {
  ...
  assert(
    ...
    `零件管理导出 content-type 必须是真实 xlsx，实际 ${contentType || '-'}`
  );
  const contentDisposition = response.headers.get('content-disposition') || '';
  assert(contentDisposition.includes('material-dashboard-export.xlsx'), '零件管理导出缺少固定响应文件名');

  const buffer = Buffer.from(await response.arrayBuffer());
  assert(buffer.subarray(0, 2).toString('utf8') === 'PK', '零件管理导出必须是 xlsx zip 文件');

  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(buffer);
  assert(workbook.worksheets.length === 1, '零件管理导出必须只导出一个工作表');
  const worksheet = workbook.worksheets[0];
  assert(worksheet.name === '零件控制面板', `零件管理导出工作表名不正确：${worksheet.name}`);
  assert(worksheet.getCell('A1').text === '零件管理控制面板导出', '零件管理导出标题不正确');
  for (const expectedScopeText of expectedScopeTexts) {
    assert(worksheet.getCell('A2').text.includes(expectedScopeText), `零件管理导出范围说明缺少 ${expectedScopeText}`);
  }
  ...
  for (const header of ['序号', '零件编码', '零件名称', '当前关系', '库存报警', 'BOM 结构', '图纸核对', 'BOM 核对', '库存核对', '状态']) {
    assert(headerValues.includes(header), `零件管理导出缺少表头 ${header}`);
  }
  return { byteLength: buffer.length, rowCount: worksheet.rowCount };
}

function assertDashboardPreviewContract(result) {
  assert(Array.isArray(result.items), '零件管理 dashboard 必须返回分页 items');
  ...
  for (const row of result.items) {
    ...
    assert(Array.isArray(details), `零件 ${row.partCode || '-'} 的 bomStructureDetails 必须是数组`);
    assert(details.length <= 10, `零件 ${row.partCode || '-'} 的 BOM 结构预览不得超过 10 条，实际 ${details.length}`);
    assert(
      typeof row.bomStructureDetailCount === 'number',
      `零件 ${row.partCode || '-'} 必须返回 bomStructureDetailCount`
    );
    assert(
      row.bomStructureDetailCount >= details.length,
      `零件 ${row.partCode || '-'} 的 bomStructureDetailCount 不得小于预览条数`
    );
    assert(
      typeof row.currentScopeBomLineCount === 'number',
      `零件 ${row.partCode || '-'} 必须返回 currentScopeBomLineCount，避免预览截断后误判是否可加入 BOM`
    );
    ...
    if (row.customerScopeKind === 'ALL') {
      ...
      assert(row.scopeType === 'COMMON', `material ${row.partCode || '-'} ALL customer scope must remain COMMON even when used by customer BOMs`);
    ...
    assert(
      historyCustomerNames.length <= 20,
      `material ${row.partCode || '-'} history customer preview must not exceed 20, actual ${historyCustomerNames.length}`
    );
    assert(typeof row.historyCustomerCount === 'number', `material ${row.partCode || '-'} must return historyCustomerCount`);
    assert(
      row.historyCustomerCount >= historyCustomerNames.length,
      `material ${row.partCode || '-'} historyCustomerCount must be >= preview length`
    );
    ...
    assert(projectModels.length <= 20, `material ${row.partCode || '-'} project model preview must not exceed 20, actual ${projectModels.length}`);
    ...
    assert(
      historyProjectModels.length <= 20,
      `material ${row.partCode || '-'} history project model preview must not exceed 20, actual ${historyProjectModels.length}`
    );
    assert(typeof row.historyProjectModelCount === 'number', `material ${row.partCode || '-'} must return historyProjectModelCount`);
    assert(
      row.historyProjectModelCount >= historyProjectModels.length,
      `material ${row.partCode || '-'} historyProjectModelCount must be >= preview length`
    );
    ...
    assert(bomNames.length <= 20, `material ${row.partCode || '-'} BOM name preview must not exceed 20, actual ${bomNames.length}`);
  ...
  assert(
    allCustomerScopeHistoryRowCount > 0,
    'material dashboard regression must include at least one 全部客户 row with order history to verify history is not expanded as scope customer'
  );
...
async function assertDashboardTestFixtureFilter() {
  ...
  assert(
    ...
    'includeTestFixtures=true must not reduce material dashboard results'
  );
...
async function assertProjectModelsTestFixtureFilter() {
  ...
  assert(
    projectModelsWithFixtures.length >= defaultProjectModels.length,
    'includeTestFixtures=true must not reduce material project model options'
  );
  return {
...
async function main() {
  let bomPreviewInfo = null;
  let bomStructureExportInfo = null;
  try {
    await cleanupDashboardBomPreviewData();
    await createDashboardBomPreviewData();
    bomPreviewInfo = await assertDashboardBomNamePreviewContract();
    bomStructureExportInfo = await assertDashboardBomStructureExportContract();
    ...
    const customerSearch = await requestJson('/customers?keyword=常州&withPage=true&limit=1&offset=0');
    ...
    assertDashboardPreviewContract(allDashboard);
    assert(
      commonDashboard.items.every((row) => row.scopeType === 'COMMON'),
      'scopeType=COMMON must only return COMMON rows'
    );
    assert(
      customDashboard.items.every((row) => row.scopeType === 'CUSTOM'),
      'scopeType=CUSTOM must only return CUSTOM rows'
    );
    assert(
      commonDashboard.items.some((row) => row.customerScopeKind === 'ALL'),
      'scopeType=COMMON must include 全部客户 common rows'
    );
    assert(
      customDashboard.items.every((row) => row.customerScopeKind !== 'ALL'),
      'scopeType=CUSTOM must not include 全部客户 common rows'
    );
    ...
    assert(
      ...
      `stockAlert=ALL 必须等同未筛选，实际 ${explicitAllDashboard.totalCount} / ${allDashboard.totalCount}`
    );
    assert(
      explicitAllStatusDashboard.totalCount === allDashboard.totalCount,
      `status=ALL 必须等同未筛选，实际 ${explicitAllStatusDashboard.totalCount} / ${allDashboard.totalCount}`
    );
