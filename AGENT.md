...
- `PRODUCTION`: 重新生产，按 `productionPlanQuantity` 生成 `ProductionTask`；生产计划数量不得小于客户订单数量。
- `STOCK`: 直接使用备货库存，`productionPlanQuantity` 必须为 0，不生成 `ProductionTask`，提交时将备货库存转成该订单待发货库存并追加 OUT / IN 流水。
- `REWORK`: 使用备货库存修改再加工，先按 `productionPlanQuantity` 扣减备货库存并追加 OUT 流水，再生成 `ProductionTask`。
- 创建或编辑 DRAFT 时可以提示库存不足但不阻止保存；提交生产时必须强制校验库存。
...
- `STOCK` 直接用库存的订单零件没有 `ProductionTask`，订单明细完成数量显示客户订单 `quantity`。
...
- 没有 `selectedStockSources` 时才允许系统自动分配；自动分配 STOCK 只能使用图号、版本、规格、厚度匹配的批次。
- “使用库存”提交生产时，订单零件必须具备 `drawingNo`、`drawingVersion`、`partSpecification`、`partThickness`；资料不完整只能保存草稿或改选库存再加工 / 重新生产。
- “使用库存”匹配库存批次时，库存来源必须能追到原图号、版本和 `drawingFileUrl`；缺失不得直接使用。
