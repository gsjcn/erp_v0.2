...
- 创建或编辑 DRAFT 时，`STOCK` / `REWORK` 可以暂时不选择库存来源并保存草稿；但只要已经选择库存批次，就必须校验批次存在、未超量和必要的人工确认记录。
...
- 生产流程配置页中，只有 `STOCK` 且 `productionPlanQuantity=0` 的零件跳过工序；部分使用库存的零件仍必须为剩余生产数量配置工序。
