...
- Web 和文档必须同步 8 个入口：客户、订单、生产流程、流程记忆、生产、统计、仓库、库存。手机端订单界面仅保留查看兜底，不作为第一阶段验收重点。
...
- 可整理第一阶段内的代码复用、注释、空状态和错误提示。手机端订单明细可查看；手机端订单列表、订单编辑、Excel 导入 / 上传界面先暂停，不做 P5 优化。
...
- 手机端订单相关界面只保留查看能力：订单明细可在手机端查看，订单列表、新增订单、订单编辑、Excel 导入 / 上传和提交生产等改动数据的能力当前以桌面端为准。已有响应式基础可以保留，但不得把手机订单编辑 / 上传作为第一阶段新增任务；手机端触发编辑类入口时必须提示到电脑端操作，防止误操作。
...
- 上述移动端响应式要求不扩大到订单新增 / 编辑 / Excel 导入上传界面；订单手机界面只保留列表和明细查看兜底，不继续做业务编辑能力。
...
- Web、Figma 和文档必须同步这 8 个入口；手机端订单业务界面只保留查看，不新增手机订单编辑 / 上传交互。
...
- `CustomerSelect.vue` 支持按客户名称、客户ID和拼音快速搜索，但不得启用 `default-first-option`；相似客户较多时必须由操作员明确选择，避免回车误选第一条客户。
- 客户搜索结果必须按命中强度排序：客户ID精确、客户名称精确、前缀、包含、拼音/首字母、联系人命中依次降低；不能只按客户ID排序导致精确客户排在后面。
...
- Excel 导入属于第一阶段“下单”能力，只允许创建或更新待提交生产的订单草稿，不得自动提交生产。
- 当前和未来都不得把 Excel 导入做成“自动提交生产”；生产提交必须由 ERP 订单页面人工触发。
- 导入入口以 Excel 文件上传为主，不依赖粘贴文本；不得设置业务字数、行数或订单数量上限。只允许保留安全性的文件大小上限，并在 UI 明确提示；大量勾选 / 排除订单号的 JSON 请求上限必须通过 `API_BODY_LIMIT` 配置，不得写死成会阻断大批量导入的默认小值。
- 必须支持连续上传：同一个导入会话可以一次多选或分多次上传文件，系统按会话合并校验，重复文件或重复业务行必须识别并提示。
- 导入记录列表和预览统计必须面向大文件优化，读取 `OrderImportRow` 时不得为了统计加载 `raw` JSON；同一订单跨文件连续上传的统计重算必须按文件上传顺序和来源行顺序处理。
- 单个 Excel 文件上传写入导入文件记录和导入行时必须使用事务；若解析或数据库写入失败，必须删除服务器端已落盘的临时 Excel 文件，避免留下半截导入记录或孤立文件。
- 未提交的导入会话必须支持删除单个已上传 Excel 文件，删除后重新计算本会话预览，不能要求用户只能整批放弃。
- 未提交的导入会话必须支持“放弃本次导入”：清空本次预览、导入行、上传文件记录，并尽量删除服务器端临时 Excel 文件；已提交会话不得放弃。
- 已提交的导入会话必须支持“删除导入记录 / 上传记忆”：只删除导入会话、导入行、上传文件记录和服务器端 Excel 文件，不得删除已经生成的订单。
- 当前正式导入工作表为 `ERP上传净表`，台账表只用于人员录入和复核。
- 后端导入必须严格读取 `ERP上传净表`，不得在缺少该工作表时回退读取第一张表，避免误把台账表或其他格式文件当作上传净表。
- `ERP上传净表` 明细区必须连续填写，数据中间不得留空行；只允许尾部空白行。台账表可以为了阅读分隔订单块，但正式上传净表必须是无空行连续数据。
- 导入时必须按后端重新解析和校验数量，不能只相信 Excel 公式结果。
- 模板可以提供自动序号、组件编号和需求数量公式，但后端导入时必须对缺失的自动序号、组件编号和需求数量做兜底计算。
- 后端兜底自动绑定所属组件时，必须按整个导入会话、同一订单编号、文件上传顺序和来源行顺序计算；只能在“上方已有最近组件、当前零件有单套用量、且当前零件没有订单数”的场景绑定，避免把普通独立零件误归到组件下。
- 订单草稿保存和提交生产前必须校验组件结构：组件行必须有唯一组件编号，零件行不能填写组件编号，子零件的所属组件必须能在当前订单内找到；组件编号保存时统一大写。
- 新增 / 编辑订单的所属组件下拉只能选择当前订单已有组件编号，不能通过 `allow-create` 创建不存在的父组件；组件行仍允许手工输入未定编号。零件行从组件行切换或新增到组件段内时，可在未手工选择所属组件的前提下自动继承上方最近组件编号。
- 新增 / 编辑订单时，如果组件行编号被手工修改并标准化，仍指向旧组件编号的子零件必须自动同步到新组件编号；已经手工指向其他组件的子零件不得被覆盖。
- 新增 / 编辑订单时，如果组件行被切换为零件行，仍指向原组件编号的子零件必须自动清空所属组件，避免留下失效父组件；指向其他组件的子零件不得被清空。
- `订单数` 表示订单层面的输入数量；`需求数量(自动)` 导入后默认写入订单零件 `quantity` 和 `productionPlanQuantity`，后续生产计划仍可在 ERP 草稿中调整。
- Excel 中的 `工艺路线`、`工艺备注` 只能作为订单草稿里的初始工序建议；最终工序以 ERP 中人工编辑和确认后的 `OrderLineProcessStep` 为准。
- 导入阶段不决定使用库存、备货、手工计划需加工数、履约方式或是否生成生产任务；这些只能在 ERP 草稿和提交生产流程中决定。
- Excel 导入创建 `DRAFT` 草稿订单时，必须把订单行中的 `partCode`、`partName`、`unit`、`partSpecification` 同步 `upsert` 到 `Material` 基础库；这只是物料基础资料入库，不代表产生库存数量。
- 导入预览、导入记录列表和导入提交响应必须返回本次可导入 / 实际同步的物料基础资料去重数量，并返回前 5 个物料号示例；前端预览统计、历史摘要和成功提示都需要展示或可查看该信息，方便操作人员核对。
- 组件行和子零件行必须保留结构字段：组件编号、所属组件编号、行类型、零件类型、Excel 自动序号、来源文件和来源行号，方便后续追溯导入来源。
- 上传文件名和来源文件名必须按 UTF-8 / multipart 兼容方式规范化，避免中文文件名在导入记录、订单明细、导入追溯和原文件预览中出现乱码。Excel 单元格中文内容必须通过 `exceljs` 正常读取，不得用错误编码二次转换。
- 订单导入弹窗必须支持上传文件级预览：上传后可按单个 Excel 文件查看原始导入行；大文件预览可以分页加载，但不得限制实际上传、解析和创建草稿订单的数据量。
- 订单详情必须支持查看来源 Excel 预览：按来源文件、工作表和 Excel 行号展示当前订单的原始导入行，并可打开服务器保存的原始 `.xlsx` 文件；删除导入记忆后订单仍保留来源文字，但应提示原文件不可预览。
- 订单导入 DTO、数据库订单导入字段和订单零件字段不得增加 `MaxLength` 或 `@db.VarChar` 等业务字数限制；允许分页预览和文件名安全处理，但不能截断 Excel 中的工艺路线、工艺备注、产品名称、图号、规格和来源信息。
- 组件编号不是全局固定编号，只在同一订单块内表达父子关系；新订单块必须从 `C001` 重新计算，ERP 导入后按订单维度保存。
- 模板默认提供 `C001-C9999` 便于选择，但业务上可能出现其他组件编号；系统不得把组件编号写死为唯一格式，Excel 下拉也不得阻止手工输入其他编号，只应校验同一订单内组件编号非空、唯一、父子关系正确。
- 导入预览必须先展示错误和警告；存在阻断错误的订单不得创建订单草稿，但允许使用者先勾选无错误订单创建草稿。
- 导入预览必须支持导出问题明细 Excel，至少包含订单编号、来源文件、来源行、自动序号、物料号、组件关系、严重程度、问题代码和问题说明，方便大批量修正原始清单。
- 导入会话提交必须是事务化的；同一次提交中选中的多个订单要么全部创建草稿并标记会话已提交，要么全部回滚，不能出现部分订单已创建、部分订单失败的半成功状态。
- 导入预览必须带 `previewToken` 或等价版本校验；提交导入必须携带该版本令牌，若缺失或会话文件列表已经变化，必须拒绝提交并要求刷新，避免未复核的新上传行被直接生成草稿订单。
- 导入会话提交后必须保存实际生成的订单号列表；导入记录列表和删除导入记忆提示应按 `sourceImportSessionId` 查询当前仍存在的订单，订单号被编辑、草稿被删除或订单被人工提交生产后不得继续用“当前草稿”误导用户。
- Excel 上传测试模板必须提供给使用人员，至少包含字段说明、上传净表和可直接上传的测试数据，避免用户不知道应上传什么格式。
- ERP 订单页面必须提供可下载的空白上传模板，模板至少包含 `ERP上传净表`、字段说明、示例数据和 `C001-C9999` 组件编号选项。
- 从 Excel 导入生成的 `DRAFT` 草稿订单必须可继续编辑；编辑时不得丢失组件编号、所属组件编号、行类型、零件类型和导入来源字段。
- 导入错误的 `DRAFT` 草稿订单必须可删除。只有未提交生产、没有生产任务、没有库存批次、没有库存流水的草稿订单允许删除；删除时必须释放该草稿订单的订单号占用。
- 已提交生产或已产生生产 / 库存记录的订单不得删除，只能按订单取消流程处理。
...
- 零件编码必须支持从 `Material` 搜索选择，也必须允许继续手工输入新编码；搜索大小写不敏感，支持 `partCode`、`partName`、规格、图号、拼音、首字母、历史客户名称 / 客户名称片段 / 客户拼音首字母 / `customerCode` / 历史订单命中。
- 新增或编辑订单时，零件编码搜索必须接收当前 `customerId`，优先展示该客户过往使用过的物料，并显示最近订单、历史客户和当前客户使用次数，方便快速复用。
- 当前客户历史物料的排序必须先按搜索命中强度，再按当前客户使用次数、当前客户最近订单日期、全局历史使用次数和可用库存排序；不能让同分旧记录长期压过该客户最近使用的物料。
- 物料建议传入 `customerId` 时，`lastCustomerOrderNo` / `lastCustomerOrderDate` 必须优先代表当前客户的最近使用记录；即使同一零件后来被其他客户使用，也不能把其他客户订单伪装成当前客户最近订单。
- 物料建议传入 `customerId` 且同一 `partCode` 有多个客户历史时，基础名称、规格、图号、版本、厚度和项目型号必须优先采用当前客户最近一次使用记录；不能因为其他客户更晚使用同一编码而覆盖当前客户识别资料。
- 新增 / 编辑订单、库存搜索和库存来源核对里的物料建议项必须显示备货库存、订单库存、单位、规格、历史使用次数和命中原因，避免同名、相似图号或不同库存来源的零件被误选。
- 物料建议项必须把图号、版本、厚度、项目型号、图纸状态等关键识别信息放在靠前位置；历史客户和历史订单信息用于辅助判断，但不能挤掉这些零件身份字段。
- 物料建议项有图纸日期时必须展示图纸日期；日期、状态和版本都属于判断是否用错图纸的身份信息。
- 物料快速搜索必须把厚度和图纸日期作为图纸资料匹配字段，支持输入 `1`、`1.0mm`、`1.50mm`、`1.5mm`，以及 `2026-05-30`、`2026/5/30`、`2026530` 这类图纸日期关键词查找历史零件。
- 物料建议项可以摘要展示历史客户列表，避免下拉框被大量客户名撑开；后端仍必须返回完整历史客户数据，不得因此限制导入或搜索数据量。
- 同一 `partCode` 在历史订单中出现多个名称、规格、图号、版本、厚度或项目型号组合时，物料建议必须提示“同编码多套历史资料”一类风险，让操作员核对图号、规格、厚度和项目型号后再选择。
- 物料建议的同编码多套历史资料风险必须返回并展示具体冲突字段；如果只有厚度不同，就提示核对厚度，不能只给泛泛的风险说明。
- 同一 `partCode` 存在多套历史识别资料时，即使当前输入只有一个精确命中，也不得在失焦时自动套用物料资料；必须保留输入并要求操作员点击候选项确认。
- 操作员人工选中同编码多套历史资料的物料后，当前订单行必须持续显示核对提醒；如果随后手工修改 `partCode` 或 `partName`，该提醒必须随旧候选一起清除。
- 订单详情页必须根据历史订单行重新计算并显示同编码多套历史资料风险，不能只依赖新增 / 编辑弹窗里的临时提示。
- 提交生产弹窗必须汇总同编码多套历史资料零件，并要求操作员勾选确认已核对图号、规格、厚度和项目型号后才能提交。
- 后端提交生产接口也必须在事务内重新计算同编码多套历史资料风险；存在风险且请求未带 `materialIdentityConfirmed` 时必须拒绝提交，不能只依赖前端勾选。
- 操作员确认同编码多套历史资料风险并成功提交生产后，后端必须把“同编码多套历史资料已核对”摘要追加到订单备注，摘要本身必须包含提交人员和前几条风险行，便于后续追溯。
- 库存来源核对弹窗里的可替代物料搜索也必须接收当前 `customerId`，用于优先展示当前客户历史用料；仍允许按编码、名称、图号、库存来源订单和批次搜索其他可替代库存。
- `sourceType=STOCK` 或 `sourceType=ORDER` 的物料建议只能返回当前范围内有可用数量的物料；普通下单搜索可以继续返回历史用料和 0 库存物料，库存来源核对不能把只有历史、没有库存的物料当成可选来源。
- 库存页的 `Material` 物料基础库搜索也必须支持历史客户、历史订单、图号和项目型号命中；操作员维护或停用物料记忆时，不能只能按编码、名称、规格查找。
- 物料搜索输入框聚焦但未输入关键字时，不得一次返回全部物料；有当前客户时优先返回该客户历史用料，没有当前客户时不返回全库基础物料，要求输入关键词后再搜索；输入关键词后返回全部命中结果，不得静默截断。
- 新增 / 编辑订单里的零件类型只能从固定类型中选择，不得 `allow-create` 创建临时类型，也不得启用 `default-first-option` 自动选第一条；成品规格可以手工输入，但同样不得启用 `default-first-option`，避免输入部分规格后回车误选系统第一条规格。
- 操作员输入完整 `partCode` 或 `partName` 后未手动选择下拉项时，系统只能在唯一精确命中时自动带出物料资料；如果多个物料同名或多个结果精确命中，必须保留手工输入并要求下单人员从下拉列表选择具体零件，避免自动套错图纸和规格。
- 库存来源核对弹窗的替代物料搜索也必须遵守唯一精确命中规则；不能因为下拉缓存里存在多个精确命中就用第一条自动切换库存来源。
- 库存来源替代物料搜索中同一 `partCode` 存在多套历史识别资料时，即使接口或缓存只有一个精确命中，也不得自动切换库存来源；必须显示候选并由操作员点击确认。
- 库存来源核对弹窗中，即使接口只返回 1 个模糊命中物料，也不能自动切换库存来源；必须显示候选并由操作员点击确认，避免把相似名称或相似图号当成准确库存。
- 多个结果精确命中时，自动带出必须优先按当前输入字段判断唯一命中；例如在 `partName` 输入框里有多个同名零件时，不得因为另一个零件的 `partCode` 也等于该输入值就自动选择编码命中的零件。
- 自动带出物料资料前必须确认当前输入值仍等于发起查询时的值；如果操作员在查询返回前已经继续修改输入，旧查询结果不得覆盖新输入。
- 选中物料后自动带出 `partCode`、`partName`、`unit`、`partSpecification`，并在订单行尚未填写图纸资料时优先带出历史订单里的 `drawingNo`、`drawingVersion`、`drawingDate`、`drawingStatus`、`partThickness`、`projectModel`；库存数量从 `InventoryBatch` 实时汇总。
- 选中历史物料后若操作员又手工修改 `partCode` 或 `partName`，前端必须清理本次选择自动带出的旧物料名称、规格、图纸资料和厚度，防止旧物料资料跟随新编码或新名称保存；操作员手工改过的字段不得被静默覆盖。
- `Material` 基础库属于下单搜索记忆，必须支持编辑和删除记忆；删除只能软停用 `Material`，不得删除历史订单、库存批次、生产任务或导入追溯。
...
- 库存来源默认按适配程度和库存数量从少到多使用；操作人员可以拖拽调整已选库存使用顺序，上移 / 下移只能作为备用操作。
...
- 拖拽排序手柄必须使用图标按钮并保留 `title` / `aria-label`，避免在紧凑工序列表里使用占宽文字按钮。
- 拖拽排序结束后必须清理工序下拉筛选关键字，避免下一次编辑时沿用上一次筛选结果。
- 左侧卡片描述使用“零件流程配置进度”。
- 保存按钮文案使用“保存零件流程”。
